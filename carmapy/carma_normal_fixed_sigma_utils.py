...
from scipy.sparse import csc_matrix, coo_matrix, vstack, csr_matrix
...
def EM_M_step_func(input_response=None, w=None, input_alpha=0.5, EM_dist="Logistic"):
    ...
    if EM_dist == "Poisson":
        ...
        cv_poisson = GridSearchCV(
            poisson_model, param_grid, cv=3, scoring="neg_mean_squared_error"
        )
        cv_poisson.fit(w, count_index)
        glm_beta = np.concatenate(
            ([cv_poisson.best_estimator_.intercept_], cv_poisson.best_estimator_.coef_)
        )
    elif EM_dist == "Logistic":
        cv_logistic = LogisticRegressionCV(
            Cs=10, cv=3, penalty="l2", scoring="neg_log_loss"
        )
...
def credible_set_fun_improved(pip, ld, true_beta=None, rho=0.99):
    ...
    """
    candidate_r = np.concatenate(
        (np.arange(0.5, 0.95, 0.05), np.arange(0.96, 1.0, 0.01))
    )
    snp_list = []
    ...
    for r in candidate_r:
        ...
        while sum(pip[pip_order[s:]]) > rho:
            ...
            if sum(pip[cor_group]) > rho:
                ...
                snp_index = [
                    cor_group[i]
                    for i in np.argsort(-group_pip)[
                        : min(
                            np.where(np.cumsum(sorted(group_pip, reverse=True)) > rho)[
                                0
                            ]
                        )
                        + 1
    ...
    if sum(len(x) for x in snp_list) != 0:
        ...
        if true_beta is not None:
            causal_snp = (
                len([x for x in true_beta if x in np.concatenate(credible_set_list)])
                if len([x for x in true_beta if x in np.concatenate(credible_set_list)])
                != 0
                else 0
            )
            return (
                [
                    causal_snp,
                    length_credible if length_credible != 0 else None,
                    np.mean([len(x) for x in credible_set_list]),
                    np.mean(purity),
                ],
                credible_set_list,
            )
...
def credible_model_fun(likelihood, model_space, bayes_threshold=10):
    ...
    credible_model_list.extend(
        (
            pd.DataFrame(
                {"Posterior.Prob": post_prob[np.where(bayes_f < bayes_threshold)[0]]}
            ),
            np.unique(input_rs),
        )
    )
...
def module_cauchy_shotgun(
    ...
    Attributes:
        ...
        lambda_val (float, optional): Regularization parameter. Default is 1.
    ...
    if model_prior == "Poisson":
        lambda_val = 1
        prior_dist = partial(poisson_prior_dist, lambda_val=lambda_val, p=p)
    ...
    # IMPORTANT: I don't calculate outliers here, I just define which is the function to use later
    if effect_size_prior == "Cauchy":
        marginal_likelihood = carmapy.carmapy_c.Cauchy_fixed_sigma_marginal
        tau_sample = np.random.gamma(0.5, scale=2, size=int(1e5))
        if outlier_switch:
    ...
    if effect_size_prior == "Spike-slab":
        ...
        if outlier_switch:
            ...
            outlier_tau = tau_sample
    print("Outlier function definition complete.")
    ...
    print(
        "null margin using prior dist:", null_margin
    )  # 0.0 output matches R equivalent
    if C_list is None:
        C_list = [[], []]
        ...
        for l in range(inner_all_iter):
            ...
            for h in range(10):
                ...
                if conditional_S is None:
                    ...
                    if len(working_S) != 0:
                        base_model = csr_matrix(
                            (
                                np.ones(len(working_S)),
                                ([0] * len(working_S), working_S - 1),
                            ),
                            shape=(1, p),
                        )
                        p_S = len(working_S)
                        base_model_margin = marginal_likelihood(
                            working_S, Sigma, z, tau_sample, p_S, y_var
                        ) + prior_dist(base_model)
                    else:
                        base_model = null_model
                        base_model_margin = null_margin
                ...
                if len(working_S) > 1:
                    ...
                    for i in range(len(set_gamma)):
                        ...
                        if len(C_list[1]) <= col_num:
                            ...
                            print("len(C_list[1]) > col_num")
                            computed_index = match_dgCMatrix(
                                C_list[1][col_num], matrix_gamma[i]
                            )
                        ...
                        if np.sum(~np.isnan(computed_index)) == 0:
                            set_gamma_margin.append(
                                np.apply_along_axis(
                                    marginal_likelihood,
                                    0,
                                    np.array([set_gamma[i]], dtype=np.int64),
                                    Sigma=Sigma,
                                    z=z,
                                    tau=tau_sample,
                                    p_S=p_S,
                            ...
                            C_list[1][col_num] = vstack(
                                (C_list[1][col_num], dense_gamma_matrix)
                            )
                            set_gamma_prior = [
                                prior_dist(row) for row in dense_gamma_matrix
                            ]
                            set_gamma_margin[-1] += set_gamma_prior[-1]
                        else:
                            set_gamma_margin.append(
                                np.full(dense_gamma_matrix.shape[0], np.nan)
                            )
                            set_gamma_margin[-1][~np.isnan(computed_index)] = C_list[0][
                                col_num
                            ][~np.isnan(computed_index)]
                            if np.sum(np.isnan(computed_index)) != 0:
                                set_gamma_margin[-1][
                                    np.isnan(computed_index)
                                ] = np.apply_along_axis(
                                    marginal_likelihood,
                                    0,
                                    set_gamma_i[np.isnan(computed_index)],
                                    Sigma=Sigma,
                                    z=z,
                                    tau=tau_sample,
                                    p_S=p_S,
                            ...
                            C_list[0][col_num].extend(
                                set_gamma_margin[-1][np.isnan(computed_index)]
                            )
                            C_list[1][col_num] = vstack(
                                (
                                    C_list[1][col_num],
                                    dense_gamma_matrix[np.isnan(computed_index)],
                                )
                            )
                            set_gamma_margin[-1] += np.apply_along_axis(
                                prior_dist, 0, dense_gamma_matrix
                            )
                ...
                if len(working_S) == 1:
                    ...
                    for i in range(1, 3):
                        ...
                        if np.sum(~np.isnan(computed_index)) == 0:
                            ...
                            print(
                                "set_gamma_margin S==1",
                                len(set_gamma_margin[0]),
                                set_gamma_margin,
                            )
                            C_list[0].extend(set_gamma_margin)
                            print("s=1 matrix_gamma[1]", matrix_gamma[i])
                            C_list[1] = vstack(
                                (C_list[1], matrix_gamma[i])
                            )  # TODO: 05/10 CODE BREAKS HERE
                            ...
                            set_gamma_margin[i] = np.full(
                                matrix_gamma[i].shape[0], np.nan
                            )
                            ...
                            if np.sum(np.isnan(computed_index)) != 0:
                                set_gamma_margin[i][
                                    ...
                                    marginal_likelihood,
                                    0,
                                    set_gamma[i][np.isnan(computed_index)],
                                    Sigma=Sigma,
                                    z=z,
                                    tau=tau_sample,
                                    p_S=p_S,
                            ...
                            C_list[0][col_num].extend(
                                set_gamma_margin[i][np.isnan(computed_index)]
                            )
                            C_list[1][col_num] = vstack(
                                (
                                    C_list[1][col_num],
                                    matrix_gamma[i][np.isnan(computed_index)],
                                )
                            )
                            for j in range(matrix_gamma[i].shape[0]):
                                row = matrix_gamma[i].getrow(j).toarray()[0]
                ...
                if len(working_S) == 0:
                    ...
                    for i in [1]:
                        ...
                        if np.sum(~np.isnan(computed_index)) == 0:
                            ...
                            set_gamma_margin[~np.isnan(computed_index)] = C_list[
                                0
                            ].extend([~np.isnan(computed_index)])
                            if np.sum(np.isnan(computed_index)) != 0:
                            ...
                            C_list[1][col_num] = vstack(
                                (
                                    C_list[1][col_num],
                                    matrix_gamma[1][np.isnan(computed_index)],
                                )
                            )
                ...
                if len([x for x in add_index if not np.isnan(x)]) > 10:
                    check_index = np.random.choice(
                        np.where(~np.isnan(add_index))[0], 10
                    )
                ...
                if len([x for x in add_index if not np.isnan(x)]) != 0:
                    B_list[0] = np.concatenate(
                        (B_list[0], add_B[0][np.isnan(add_index)])
                    )
                    B_list[1] = vstack(
                        (
                            B_list[1],
                ...
                if len(working_S) != 0:
                    ...
                    for i in range(1):
                        ...
                        set_star["gamma_set_index"][i] = np.random.choice(
                            range(1, len(set_gamma_margin[i]) + 1), 1, p=np.exp(aa)
                        )
                        set_star["margin"][i] = set_gamma_margin[i][
                            set_star["gamma_set_index"][i]
                        ]
                    ...
                    if outlier_switch:
                        for i in range(1, len(set_gamma)):
                            while True:
                                ...
                                aa = aa.flatten()
                                set_star["gamma_set_index"][i] = np.random.choice(
                                    range(len(set_gamma_margin[0])), 1, p=np.exp(aa)
                                )
                                set_star["margin"][i] = set_gamma_margin[
                                    set_star["gamma_set_index"][i]
                                ]
                                test_S = set_gamma[i][set_star["gamma_set_index"][i]]
                                # Added these 2 lines to correct input for ridge_fun()
                                test_S = np.concatenate(
                                    [np.array(sublist).flatten() for sublist in test_S]
                                )
                                ...
                                if len(test_S) > 1:
                                    ...
                                    opizer = optimize(
                                        ridge_fun, interval=[0, 1], maximum=True
                                    )
                                    modi_ld_S = opizer["maximum"] * modi_ld_S + (
                                        1 - opizer["maximum"]
                                    ) * np.diag(np.diag(modi_ld_S))

                                    modi_Sigma[test_S, test_S] = modi_ld_S

                                    test_log_BF = outlier_likelihood(
                                        test_S, Sigma, z, outlier_tau, len(test_S), 1
                                    ) - outlier_likelihood(
                                        test_S,
                                        modi_Sigma,
                                        z,
                                ...
                                if np.exp(test_log_BF) < outlier_BF_index:
                                    ...
                                    set_gamma[i] = set_gamma[i][
                                        ~np.isin(
                                            set_gamma[i], set_star["gamma_set_index"][i]
                                        )
                                    ]
                                    set_gamma_margin[i] = set_gamma_margin[i][
                                        ~np.isin(
                                            set_gamma_margin[i],
                                            set_star["gamma_set_index"][i],
                                        )
                                    ...
                                    conditional_S = np.concatenate(
                                        (
                                            conditional_S,
                                            test_S[~np.isin(test_S, working_S)],
                                        )
                                    )
                                    conditional_S = np.unique(conditional_S)
                                else:
                    ...
                    if len(working_S) == num_causal:
                        ...
                        aa = (
                            set_star["margin"]
                            - current_log_margin
                            - max(set_star["margin"] - current_log_margin)
                        )
                        sec_sample = np.random.choice([1, 3], 1, p=np.exp(aa))
                        S = set_gamma[sec_sample][
                            set_star["gamma_set_index"][
                                np.where(sec_sample == set_star["set_index"])[0][0]
                            ],
                            :,
                        ]
                    else:
                        ...
                        S = set_gamma[sec_sample][
                            set_star["gamma_set_index"][sec_sample], :
                        ]
                else:
            ...
            if conditional_S is not None:
                ...
                for tt in conditional_S:
                    c_index = (
                        B_list[1].indices[
                            B_list[1].indptr[tt] : B_list[1].indptr[tt + 1]
                        ]
                        + 1
                    )
                ...
                temp_B_list = []
                temp_B_list.append(
                    B_list[0][np.setdiff1d(range(len(B_list[0])), all_c_index)]
                )
                temp_B_list.append(
                    B_list[1].tocsr()[
                        np.setdiff1d(range(B_list[1].shape[0]), all_c_index), :
                    ]
                )
            else:
            ...
            if num_causal == 1:
                ...
                single_marginal = np.apply_along_axis(
                    marginal_likelihood,
                    0,
                    single_set,
                    Sigma=Sigma,
            ...
            if conditional_S != None:
                conditional_S_list = pd.DataFrame(
                    {"Index": conditional_S, "Z": z[conditional_S, :]}
                )

            stored_bf = 0

            if output_labels is not None and not os.path.exists(output_labels):
            ...
            difference = abs(
                np.mean(
                    result_B_list[0][
                        : round(
                            np.quantile(np.arange(1, len(result_B_list[0]) + 1), q=0.25)
                        )
                    ]
                )
                - stored_bf
...
def input_prior_dist(x):
    ...
    if len(variable_index) > 0:
        return (
            np.sum(posi_log_pro[variable_index])
            + np.sum(nega_log_pro[other_index])
            - np.sum(nega_log_pro)
        )
    else:
...
def poisson_prior_dist(t, lambda_val, p):
    """Calculate the Poisson prior distribution for a given input vector `t`."""
    dim_model = np.sum(t).item()
    return dim_model * log(lambda_val) + gammaln(p - dim_model) - gammaln(p)
...
def duplicated_dgCMatrix(dgCMat, MARGIN):
    ...
    if MARGIN == 1:
        ...
        result = np.array(
            [np.any(np.concatenate(RowLst[i])) for i in range(len(RowLst))]
        )
    elif MARGIN == 2:
        ...
        result = np.array(
            [np.any(np.concatenate(ColLst[i])) for i in range(len(ColLst))]
        )
    else:
...
def index_fun_inner(x, max_col_index):
    ...
    m = coo_matrix(
        (data, (row_indices, col_indices)), shape=(n_rows, max_col_index + 1)
    ).tocsc()
    return m
...
def index_fun(outer_x, Max_Model_Dimins=10):
    ...
    if n_rows > 1000:
        index_bins = np.arange(
            n_rows // Max_Model_Dimins, n_rows, n_rows // Max_Model_Dimins
        )
        ...
        for i in range(len(index_bins) - 1):
            result_m = vstack(
                [
                    result_m,
                    index_fun_inner(
                        outer_x[index_bins[i] : index_bins[i + 1], :], max_col_index
                    ),
                ]
            )
        if index_bins[-1] != n_rows:
            result_m = vstack(
                [result_m, index_fun_inner(outer_x[index_bins[-1] :, :], max_col_index)]
            )
    else:
...
def ridge_fun(
    x, modi_ld_S, test_S_indices, temp_Sigma, z, outlier_tau, outlier_likelihood
):
